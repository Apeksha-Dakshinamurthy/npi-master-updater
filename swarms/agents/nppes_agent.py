...
from ..prompts.nppes_prompt import NPPES_AGENT_PROMPT
...
class NPPESAgent:
    def __init__(self):
        ...
        self.agent = create_react_agent(
            ...
            prompt=NPPES_AGENT_PROMPT
        )

    def process(self, input_data: dict) -> dict:
        search_query_input = input_data.get("search_query_input", "")
        query = f"Search NPPES and state medical boards for provider: {search_query_input}"
            ...
            for message in result['messages']:
                ...
                if content:
                    if isinstance(content, str) and content.strip():
                        nppes_results.append(content)
                    elif isinstance(content, list):
                        # Convert list to string
                        content_str = str(content)
                        if content_str.strip() and content_str != '[]':
                            nppes_results.append(content_str)
                    else:
        ...
        return {"nppes_results": nppes_results, "input": input_data}
